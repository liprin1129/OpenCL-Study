...
#print "RESULT: {0} in {1}s".format(np.reshape(mat_c, (1000, 1000)), run_time-start_time)
...
__kernel void dotProduct(
    ...
    __global float* outputC) {
        ...
        outputC[gid] = dot(inputA[gid], inputB[0]);
    }
'''

mat_a = np.random.randn(1000*1000)
mat_b = np.random.randn(1000)
mat_c = np.empty(1000)
...
program.dotProduct(queue, mat_c.shape, None, buffer_a, buffer_b, buffer_c)
